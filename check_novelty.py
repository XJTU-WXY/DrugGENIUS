...
def main():
    ...
    for f in tqdm(files, desc="Checking similarities"):
        prop = load_json(f)
        if args.device == "cpu":
            sims = fpe.similarity(prop["SMILES"], threshold=args.threshold, metric='tanimoto', n_workers=args.proc)
        else:
            sims = fpe.similarity(prop["SMILES"], threshold=args.threshold)
        np.savetxt(os.path.join(args.input, f"{prop['Hash']}_sim.csv"), sims, delimiter=",", header="ID,Similarity", comments='', fmt = ['%s', '%.8f'])
